...
러스트의 *클로저*는 변수에 저장하거나 다른 함수에 인수로 넘길 수 
...
제외하고는 여러분이 이미 배운 개념만 허용했습니다. `giveaway` 메서드에서는
...
`unwrap_or_else`의 인수로는 `|| self.most_stocked()`이라는 클로저
...
여기서 한 가지 흥미로운 관점은 현재의 `Inventory` 인스턴스 상에서
...
반환 값의 타입을 `fn` 함수에서처럼 명시하도록 요구하지 않습니다.
...
보일 것입니다. 이 예제에서는 예제 13-1에서처럼 인수로 넘기고 싶은
...
첫 번째 줄은 함수의 정의를 보여주고, 두 번째 줄은 모든 것이 명시된
클로저 정의를 보여줍니다. 세 번째 줄에서는 타입 명시를 제거했습니다.
...
클로저 정의에 대하여, 컴파일러는 각각의 매개변수와 변환값마다 하나의
...
클로저는 캡처된 값이 어떻게 쓰이는지에 기초하여 어떤 방법을 쓸지
결정할 것입니다.
...
때는 다른 빌림이 허용되지 않기 때문입니다. `println!`을 추가해서 어떤
에러가 나오는지 시도해 보세요!
...
이 기술은 클로저를 새 스레드에 넘길 때 데이터를 이동시켜서 새로운 스레드가
이 데이터를 소유하도록 하는 데 대체로 유용합니다. 스레드가 무엇이고 왜 이를
...
하는 슬라이스 내에서 현재 아이템에 대한 참조자를 하나의 인수로 받아서,
...
`sort_operations` 벡터에 밀어 넣는 형태로 시도하고 있습니다. 클로저는
`value`를 캡처하여 `sort_operations` 벡터로 소유권을 이전시키는 것으로
`value`를 클로저 밖으로 이전시킵니다. 이 클로저는 한 번만 호출될 수 있습니다;
두 번째 호출 시도에서는 `value`가 더 이상 이 환경에 남아있지 않은데
...
클로저 인수를 받으니, 계속하려면 이 클로저에 대한 세부적인 내용을
새겨둡시다!
