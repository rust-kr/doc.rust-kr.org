...
불변으로 빌려오기, 가변으로 빌려오기, 그리고 소유권 이전이죠.
...
따라서 컴파일러는 `list`를 새 스레드에 제공될 클로저로 이전시켜
...
구현하는 트레잇에 영향을 주고, 트레잇은 함수와 구조체가 사용할
수 있는 클로저의 종류를 명시할 수 있는 방법입니다. 클로저는
클로저의 본문이 값을 처리하는 방식에 따라서 이 `Fn` 트레잇들 중 하나,
...
r.width`는 자신의 환경으로부터 어떤 것도 캡처나 변형, 혹은 이전을 시키지
