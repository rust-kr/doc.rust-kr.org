...
다만 이 코드는 컴파일되지 않으니 유의해 주세요! 에러 해결법은 이번 장에서 천천히 알아보겠습니다.
...
도움말의 제안을 따라서 `T`가 오직 `PartialOrd`를 구현한 것일 때만 유효하도록
...
컴파일러는 우리가 `x`에 정숫값 5를 대입할 때 `Point<T>` 인스턴스의
...
`p3`는 `p1` 상에서 `p2`를 인수로 `mixup` 메서드를 호출해 반환된 값입니다.
