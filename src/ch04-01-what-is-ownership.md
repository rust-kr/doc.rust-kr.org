...
해제하는 방식을 택했습니다. 이때 러스트는 제3의 방식을 택했습니다:
...
더 안정적이고 더 효율적인 코드를 쉽게 작성하는 방법을 찾게 될 테니, 포기하지 마세요!
...
> 스택 대신 힙에 저장되어야 합니다.
> 
> 힙은 스택보다 복잡합니다. 데이터를 힙에 넣을 때 먼저 저장할 공간이 있는지
> 운영체제에 물어봅니다. 그러면 메모리 할당자는 커다란 힙 영역 안에서 어떤 빈
...
> 메모리 할당자가 데이터를 저장하기 충분한 공간을 먼저 찾고
...
> 점을 알고 있으면 소유권의 동작 방식을 이해하는 데 도움이 됩니다.
...
네임스페이스는 7장 [‘경로를 사용하여 모듈 트리의 아이템 참조하기’][paths-module-tree]<!-- ignore -->
에서 다루고 있습니다.
...
> Note: C++에서는 이런 식으로 아이템의 수명이 끝나는 시점에 리소스를 해제하는
...
<span class="caption">예제 4-2: 변수 `x`의 정숫값을
`y`에 대입하기</span>
...
`s2`에 `s1`를 대입하면 `String` 데이터가 복사됩니다.
...
더 이상 유효하지 않다고 판단합니다. 이로써 러스트는 `s1`이 스코프를 벗어나더라도
아무것도 해제할 필요가 없어집니다. `s2`가 만들어진 이후에 `s1`을 사용하는 경우
...
<span class="caption">그림 4-4: `s1`이 무효화된 후의
메모리 구조</span>
...
아직 다루지 않은 부분이 있습니다.
다음 코드는 앞서 예제 4-2에서 본 정수형을 이용하는 코드입니다
(정상적으로 작동합니다):
...
[‘파생 가능한 트레이트’][derivable-traits]<!-- ignore -->를 참고 바랍니다.
...
비슷한 주석이 달린, 어떤 값을 반환하는 함수에 대한 예제 4-4를
살펴봅시다:
