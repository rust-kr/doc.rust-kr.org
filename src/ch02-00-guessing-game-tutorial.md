...
아래 예제처럼 카고를 이용하여 새로운 프로젝트를 생성합니다.
...
`String` 타입 위에 만들어진 함수입니다. 이 `new` 함수는 비어있는 새 문자열을
...
메서드를 호출합니다. 여기에 `read_line`의 인자로서
`&mut guess`를 넘겨서 사용자 입력이 저장될 문자열이
...
`Result` 타입의 값들은 다른 타입들처럼 메서드들을 가지고 있습니다.
`Result` 인스턴스는 [`expect` 메서드][expect]<!-- ignore -->를
가지고 있습니다. 만약 `Result` 인스턴스가 `Err`일 경우 `expect`
메서드는 프로그램이 작동을 멈추게 하고 `expect`에 인자로 넘겼던
...
출력할 때는 빈 중괄호들를 형식 문자열에 위치시키고, 그 뒤에 쉼표로 구분된
...
카고의 외부 크레이트 조정은 정말 멋진 부분입니다. `rand`를 사용하는
코드를 작성하기 전에 *Cargo.toml* 을 수정하여 `rand` 크레이트를 의존
리스트에 추가해야 합니다. 파일을 열고 카고가 여러분을 위해 생성한
...
각각의 요구 버전을 카고에게 알려주게 됩니다. 지금의 경우 우리는 `rand`
크레이트의 유의적 버전인 `0.8.5`을 지정했습니다. 카고는 버전을 명시하는
...
레지스트리를 업데이트하면 카고는 `[dependencies]` 절을 확인하고
...
줄 외엔 어떠한 결과도 얻지 못할 것입니다. 카고는 이미 의존 패키지들을
다운받고 컴파일했음을 알고 있고 여러분이 *Cargo.toml* 을
변경하지 않은 것을 알고 있습니다. 또한 카고는 코드가
...
보여줍니다. 의존 패키지가 변경되지 않았으므로 카고는 이미 다운받고 컴파일된 것들을
재사용할 수 있음을 알고 있습니다.
...
카고는 여러분이 다른 의존성을 추가하지 전까지는 여러분이 명시한 의존
...
만약 여러분이 미래에 프로젝트를 빌드할 경우 카고는 모든 버전들을
...
만약 당신이 *정말* 크레이트를 업데이트하고 싶은 경우를 위해 카고는 `update` 명령어를
...
다음번에 여러분이 `cargo build`를 실행하면 카고는 가용 가능한 크레이트들의
레지스트리를 업데이트할 것이고 여러분의 `rand`요구사항을 새롭게 명시한 버전에
따라 재계산할 것입니다.

[카고][doccargo]<!-- ignore -->와 [그의 생태계][doccratesio]<!-- ignore -->에
대해 더 많은 것들은 14장에서 다뤄지지만 지금 당장은 이 정도만 알면 됩니다.
카고는 라이브러리의 재사용을 쉽게 하여
...
다음으로 우리는 `gen_range` 메서드를 호출합니다.
이 메서드는 `Rng` 트레잇에 정의되어 있으므로 `use rand::Rng;`
구문을 통해 스코프로 가져올 수 있습니다. `gen_range` 메서드는
...
> Note: 크레이트에서 트레잇과 메서드, 함수중 어떤 것을 호출해야
> 할지 모를 수도 있으므로, 각 크레이트는 사용법을 담고 있는
> 문서를 갖추고 있습니다. 카고의 다른 멋진 기능은
...
마지막에 추가 했습니다. `cmp` 메서드는 두 값을 비교하며
비교 가능한 모든 것들에 대해 호출할 수 있습니다.
이 메서드는 비교하고 싶은 것들의 참조자를 받습니다.
...
`trim` 메서드는 처음과 끝 부분의 공백문자들을 제거하는데, 이는 숫자형 데이터만
...
[문자열의 `parse` 메서드][parse]<!-- ignore -->는
문자열을 숫자형으로 파싱합니다. 이 메서드는 다양한 종류의 정수형을
...
ignore -->에서 `parse` 메서드에 대해 논의할 때 언급한 방법으로
