...
지금으로서는 실제 IP 주소 *데이터*를 저장할 방법이 없습니다. 단지 어떤 *종류*
인지만 알 뿐입니다. 5장에서 구조체에 대해 방금 공부했다고 한다면,
이 문제를 예제 6-1에서 보이는 것처럼 풀려는 유혹을 받을지도
모릅니다:
...
예를 들어 비어있지 않은 리스트의 첫 번째 아이템을 요청한다면 값을 얻을 수 있을
것입니다. 그렇지만 비어있는 리스트로부터 첫 번째 아이템을 요청한다면 아무 값도
...
볼 수 있는 널 (null) 개념이 없습니다. *널*은 값이 없다는 것을 표현하는 하나의 값입니다.
널 개념이 존재하는 언어에서, 변수의 상태는 둘 중 하나입니다.
널인 경우와, 널이 아닌 경우죠.

널을 고안한 토니 호어 (Tony Hoare) 의 널 참조: 10억 달러짜리 실수
(Null References: The Billion Dollar Mistake)”에서 다음과 같이 말합니다:

> 저는 그걸 10억 달러짜리 실수라고 부릅니다. 그 당시 객체 지향 언어에서
> 참조자를 위한 첫 포괄적인 타입 시스템을 디자인하고 있었습니다.
> 제 목표는 컴파일러에 의해 자동으로 수행되는 체크를 통해 모든
> 참조자의 사용이 절대로 안전함을 보장하는 것이었습니다.
> 하지만 구현이 무척 간단하다는 단순한 이유로 널 참조를 넣고
> 싶은 유혹을 참을 수 없었습니다. 이것은 수없이 많은 에러와
> 취약점, 시스템 종료를 유발했고, 아마도 지난 40년간 10억 달러 수준의
> 고통과 손실을 초래해왔습니다.
...
`None` 값을 얻게 되면, 얻은 값이 유효하지 않은 값이라는, 어떤 면에서는 널과 같은 의미를 갖습니다.
그렇다면 왜 `Option<T>`가 널을 갖는 것보다 나을까요?

간단하게 말하면, `Option<T>`와 `T`(`T`는  어떤 타입이던 될 수 있음)는 다른 타입이며,
컴파일러는 `Option<T>` 값을 명확하게 유효한 값처럼 사용하지 못하도록 합니다.
예를 들면, 아래 코드는 `Option<i8>` 에 `i8`을 더하려고 하므로
컴파일되지 않습니다:
...
코드에 더 확신을 갖게 됩니다. 널일 수 있는 값을 사용하기 위해서,
명시적으로 값의 타입을 `Option<T>`로 만들어 줘야 합니다. 그다음엔 값을 사용할
때 명시적으로 널인 경우를 처리해야 합니다.
