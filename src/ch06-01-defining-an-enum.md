...
`IpAddrKind`이라는 열거형을 정의하면서 포함할 수 있는 IP 주소인 `V4`와 `V6`를
...
이제 `IpAddrKind`은 코드 어디에서나 쓸 수 있는 커스텀 데이터 타입이 되었습니다.
...
`V6`를 값으로 갖고, 연관된 주소로 `::1`를 갖습니다. `kind`와 `address`의
...
[표준 라이브러리에 정의된 것을 사용할 수 있습니다!][IpAddr]<!-- ignore -->
...
[표준 라이브러리에 정의되어 있습니다][option]<!-- ignore -->:
...
바꿔 말하면, `T`에 대한 연산을 수행하기 전에 `Option<T>`를 `T`로
...
그러니 한번 [문서에서][docs]<!-- ignore --> 여러분에게 필요한 메서드를
