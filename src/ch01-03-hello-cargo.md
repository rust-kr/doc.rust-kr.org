...
카고 (Cargo) 는 러스트 사용자라면 대부분 사용하는 러스트 빌드 시스템 및
...
“Hello, world!” 프로젝트를 만들면서도 카고 기능 중 코드를 생성하는 기능만 사용했었죠.
하지만 훗날 복잡한 프로그램을 작성하게 되면 이야기가 다를 겁니다.
그렇더라도, 프로젝트를 생성할 때부터 카고를 이용하면
의존성을 추가할 일이 생겨도 간단히 해결할 수 있을 겁니다.

러스트 사용자 대부분이 사용하는 툴인 만큼,
이후 내용도 여러분이 카고를 사용한다는 전제로 작성했습니다.
[“러스트 설치”][installation]<!-- ignore --> 절을 따라 하셨다면
이미 카고가 설치돼있을 테니 따로 설치하실 필요는 없으나,
...
것입니다. 여러 개의 크레이트로 구성된 복잡한 프로젝트에서는 카고가 빌드를
조정하게 하는 것이 훨씬 쉽습니다.

`hello_cargo` 프로젝트는 단순하지만, 이미 여러분은 앞으로 러스트를 사용하며
쓰게될 카고 명령어 중 대부분을 써본 것과 다름없습니다. 실제로 기존에 있던
...
더 자세한 내용은 [카고 문서 (영문)][cargo] 에서 읽어보세요.
