...
라이브러리로부터 덜 지원을 받습니다; 예를 들면 해쉬맵을 생성하는 기본 제공 매크로가 없습니다.
...
여기서 `score`는 블루 팀과 연관된 값을 가지고 있을 것이고, 결괏값은
...
우리가 옐로 팀에 대한 키가 연관된 값을 가지고 있는지 검사하고 싶다고
...
값을 갱신하는 것입니다. 예를 들어, 예제 8-25는 어떤 텍스트 내에 각 단어가
...
반복처리가 임의 순서로 일어난다고 한 것을 상기해 봅시다.
...
불리는 해쉬 함수를 사용합니다[^siphash]<!-- ignore -->. 이는 사용
가능한 가장 빠른 해쉬 알고리즘은 아니지만, 성능을 떨어트리면서 더 나은
보안을 취하는 거래는 가치가 있습니다. 만일 여러분이 여러분의 코드를
프로파일링하여 기본 해쉬 함수가 여러분의 목표에 관해서는 너무 느리다면,
...
접근하고, 수정하고 싶은 곳마다 필요한 수많은 기능들을 제공해 줄 것입니다.
...
* 해쉬맵과 벡터를 이용하여, 사용자가 회사 내의 부서에 대한 피고용인 이름을
  추가할 수 있도록 하는 텍스트 인터페이스를 만들어 보세요. 예를 들어 "Add Sally
  to Engineering"이나 "Add Amir to Sales" 같은 식으로요. 그 후 사용자가
  알파벳 순으로 정렬된 모든 사람에 대한 리스트나 부서별 모든 사람에 대한 리스트를
  조회할 수 있도록 해보세요.

표준 라이브러리 API 문서는 이 연습문제들에 대해 도움이 될 만한 벡터, 문자열,
그리고 해쉬맵의 메서드들을 설명해 줍니다!
