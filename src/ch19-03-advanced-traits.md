...
타입 `Item`은 플레이스홀더이고, `next` 메서드의 정의는 `Option<Self::Item>`
...
### 모호성 방지를 위한 완전 정규화 (fully qualified) 구문: 같은 이름의 메서드 호출하기

러스트에서는 어떤 트레잇에 다른 트레잇의 메서드와 같은 이름의 메서드가
있는 것을 막지 않으며, 한 타입에서 두 트레잇을 모두 구현하는 것도 막지
않습니다. 또한 트레잇의 메서드와 이름이 같은 메서드를 타입에 직접
구현하는 것도 가능합니다.
...
`fly` 메서드는 `self` 매개변수를 취하기 때문에, 하나의 *트레잇*을 구현하는
...
구현된 `Animal` 트레잇의 `baby_name` 메서드를 호출하고 싶음을
...
인수들의 목록만 있을 뿐입니다. 함수나 메서드를 호출하는 곳이라면
...
`outline_print` 메서드가 있는 `OutlinePrint` 트레잇을 만들고
...
`outline_print` 메서드의 구현에서 `Display` 트레잇의 기능을
