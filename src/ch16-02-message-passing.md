...
보내고자 하는 데이터와 함꼐 송신자의 메소드를 호출하면, 다른 곳에서는
...
스트링을 전송하게 하여 생성된 스레드가 메인 스레드와 통신하도록
...
소유할 필요가 있습니다. 송신 단말에는 보내고 싶은 값을 받는 `send` 메소드가
있습니다. `send` 메소드는 `Result<T, E>` 타입을 반환하므로, 수신 단말이
...
수신자는 두 개의 유용한 메소드를 가지고 있습니다: `recv`와 `try_recv`입니다. 여기서는
...
그 파라미터의 소유권을 가져가고, 이 값이 이동되면, 수신자가 이에 대한
...
이번에는 생성된 스레드가 메인 스레드로 보내고 싶은 스트링의 벡터를 
