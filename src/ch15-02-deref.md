...
그리고서 러스트의 *강제 역참조 (deref corecion)* 기능과 이 기능이
...
### 함수와 메서드를 이용한 암묵적 강제 역참조

*강제 역참조 (deref coercion)* 는 `Deref`를 구현한 어떤 타입의 참조자를
다른 타입의 참조자로 바꿔줍니다. 예를 들어, 강제 역참조는 `&String`을 `&str`로
바꿔줄 수 있는데, 이는 `String`의 `Deref` 트레이트 구현이 그렇게 `&str`을
반환하도록 했기 때문입니다. 강제 역참조는 러스트가 함수와 메서드의 인수에
...
강제 역참조는 함수와 메서드 호출을 작성하는 프로그래머들이 `&`와 `*`를
사용하여 수많은 명시적인 참조 및 역참조를 추가할 필요가 없도록 하기 위해
도입되었습니다. 또한 강제 역참조 기능은 참조자나 스마트 포인터 둘 중 어느
경우라도 작동되는 코드를 더 많이 작성할 수 있도록 해 줍니다.

강제 역참조가 실제 작동하는 것을 보기 위해서, 예제 15-8에서
...
호출될 수 있습니다. 예제 15-12에서 보는 바와 같이, 강제 역참조는 `MyBox<String>`
...
<span class="caption">예제 15-12: 강제 역참조에 의해 작동되는,
...
만일 러스트에 강제 역참조가 구현되어 있지 않았다면, `&MyBox<String>`
...
<span class="caption">예제 15-13: 러스트에 강제 역참조가 없었을 경우
작성했어야 할 코드</span>

`(*m)`은 `MyBox<String>`을 `String`으로 역참조해 줍니다. 그런 다음 `&`과
`[..]`가 전체 문자열과 동일한 `String`의 문자열 슬라이스를 얻어와서 `hello`
시그니처와 일치되도록 합니다. 강제 역참조가 없는 코드는 이 모든 기호가 수반된
상태가 되어 읽기도, 쓰기도, 이해하기도 더 힘들어집니다. 강제 역참조는 러스트가
...
컴파일 타임에 분석되므로, 강제 역참조의 이점을 얻는 데에 관해서 어떠한
런타임 페널티도 없습니다!

### 강제 역참조가 가변성과 상호작용하는 법
...
동일한 강제 역참조가 가변 참조자에 대해서도 발생함을 기술합니다.
