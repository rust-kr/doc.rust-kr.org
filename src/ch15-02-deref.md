...
`deref` 메서드의 본문은 `&self.0`로 채워졌으므로 `deref`는
...
반환하도록 했기 때문입니다. 역참조 강제는 러스트가 함수와 메서드의 인자에
대해 수행해주는 편의성 기능이고, `Deref` 트레잇을 구현한 타입에 대해서만
동작합니다. 이는 어떤 특정한 타입값에 대한 참조자를 함수 혹은 메서드의
인자로 넘기는데 이 함수나 메서드의 정의에는 그 매개변수 타입이 맞지
않을때 자동적으로 발생합니다. 일련의 `deref` 메서드 호출이 인자로 제공한
