...
Cargo가 테스트를 컴파일하고 실행했습니다. `running 1 test` 줄이
...
코드에 버그를 집어넣어서 `assert_eq!`가 실패했을 때는 어떻게 보여지는지 확인해봅시다.
...
몇몇 프로그래밍 언어, 프레임워크에서는 동치 단언 함수의 매개변수를
...
연산자를 사용합니다. 단언에 실패할 경우, 매크로는 인자를 디버그 포매팅으로
...
연산이 `Err` variant를 반환하는 것을 단언하기 위해서는 `Result<T, E>` 값에
