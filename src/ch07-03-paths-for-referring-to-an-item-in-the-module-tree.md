...
`hosting` 모듈이 비공개임을 의미하던 예제 7-4 에러로 돌아와 보죠.
...
> 시작하기 위한 양만큼의 코드가 바이너리 크레이트에 담긴 형태가 될 것입니다.
...
> 패키지 이름으로 시작하는 경로를 사용함으로써 어떠한 공개 아이템이라도 사용 가능합니다.
...
한번 `seasonal_fruit` 필드를 수정하는 코드의 주석을 해제하고 어떤 에러가 발생하는지 확인해 보세요!
...
열거형의 모든 배리언트에 대해 전부 `pub`을 붙이는 것은 짜증나는
