...
여러분이 항목을 정의하는 코드와 항목을 사용하는 코드를 분리하고 싶은지,
...
경로는 절대 경로입니다. 항목을 정의하는 코드와 호출하는 코드는 분리되어 있을
가능성이 높기 때문입니다.
...
러스트에서는 (함수, 메소드, 구조체, 열거형, 모듈, 그리고 상수 등) 모든
...
러스트에서는 `pub` 키워드를 사용하여 자식 모듈의 내부 요소를
