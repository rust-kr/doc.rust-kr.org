...
그리고 `main` 함수 내에서는 `rect1`을 인수로 넘겨 `area` 함수를 호출했었는데,
...
`Self`의 인스턴스를 빌려온다는 것을 나타내기 위해서는 `self` 축약형 앞에 계속
...
반환 값은 Boolean 타입이 될 거고, `self`의 너비, 높이가
