...
메세지를 전송하는 것에 대한 메커니즘을 제공할 예정입니다: 이 어플리케이션은 메세지를
...
불변 참조자와 메세지의 텍스트를 인자로 갖는 `send`라는 메서드 하나를 갖고
...
`send` 메서드의 정의 부분에서는 매개변수로 넘겨진 메세지를 가져와서
...
메서드를 80 값으로 호출하였는데, 이는 75 퍼센트 이상입니다. 그 다음
...
`send` 메서드의 구현부에서 첫번째 매개변수는 여전히 `self`의 불변 빌림
...
`RefCell<T>`으로는 `borrow`와 `borrow_mut` 메서드를 사용하는데,
...
가지고 있는 `Cons` 배리언트로 `List`를 생성하여 `a`에 넣었습니다. `value`은
...
`RefCell<T>`이 멀티스레드 코드에서는 동작하지 않음을 주의하세요!
