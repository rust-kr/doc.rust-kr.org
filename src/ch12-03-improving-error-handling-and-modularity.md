...
세번쨰 문제는 파일 읽기 실패 시 에러 메세지 출력을 위해서 `expect`를
...
길은 그 값에 대하여 `clone` 메소드를 호출하는 것입니다. 이는 데이터의 전체
...
> ### `clone`을 사용한 절충안
>
> 러스트인들 중 많은 이들이 런타임 비용의 이유로 `clone`을 사용한
> 소유권 문제 해결을 회피하는 경향이 있습니다. [13장][ch13]<!-- ignore -->에서
> 이러한 종류의 상황에서 더 효율적인 메소드를 사용하는 방법을 배울 것입니다.
> 하지만 지금으로서는 계속 진행하기 위해 약간의 문자열을 복사하는
> 정도는 괜찮은데, 이 복사가 딱 한번만 일어나고 파일 경로와 질의
> 문자열이 매우 작기 떄문입니다. 한번에 매우 최적화된 코드 작성을
...
들어있는 경우에는 인덱스 1이나 2의 원소에 접근을 시도하는 것이 프로그램의
...
[`Result`를 반환하는][ch9-result]<!-- ignore --> 기술을 사용해보겠습니다.
...
줄 수 있습니다. 그러면 `main`을 수정하여 `Err` variant를 사용자에게
...
두번째로 [9장][ch9-question-mark]<!-- ignore -->에서 이야기했던
...
메소드, 그리고 `run` 함수에 대해 사용했지요. 이제 우리는 테스트 해볼 수 있는
