...
우리의 프로그램을 개선하기 위해서 프로그램의 구조 및 에러 처리 방법을
위해 필요한 네 가지 문제를 고치려고 합니다. 첫 번째는, `main` 함수가 지금 두
가지 일을 수행한다는 것입니다: 인수 파싱, 파일을 읽는 작업 말입니다. 이 프로그램이
...
또한 이 주제는 두 번째 문제와 엮입니다: `query`와 `file_path`가 프로그램의
...
사용했는데, 이 에러 메시지가 고작 `Should have been able to read the
...
사용자에게 의미 있을 메시지를 출력하는 중이라는 것을 확신시켜 줄 것입니다.

이 프로젝트를 리팩터링하여 위 네 가지 문제를 해결해 봅시다.
...
인덱스 1번의 인수값을 `query` 변수에 할당하고 인덱스 2번의
인수값을 `file_path` 변수에 할당하는 대신, 전체 벡터를
`parse_config` 함수에 넘깁니다. 그러면 `parse_config`
함수는 어떤 인수값이 어떤 변수에 들어갈지 정하는 로직을
담고 있고 이 값들을 `main`에게 다시 넘겨줍니다. 여전히 `query`와
`file_path` 변수는 `main` 안에서 만들지만, `main`은 더 이상
커맨드 라인 인수와 변수들이 어떻게 상관되어 있는지를 결정할
책임을 지지 않습니다.
...
정의했습니다. `main` 안에 있는 `args` 변수는 인수값들의 소유자이고
...
복사본을 만들어 `Config` 인스턴스가 이를 소유할 수 있게 해 줄 것인데, 이는
문자열 데이터에 대한 참조자를 저장하는 것에 비해 더 많은 시간과 메모리를 소비합니다.
그러나 값의 복제는 또한 코드를 매우 직관적으로 만들어 주는데, 그 이유는
...
> 소유권 문제 해결을 회피하는 경향이 있습니다. [13장][ch13]<!-- ignore -->에서
...
> 시도하기보다는 다소 비효율적이라도 동작하는 프로그램을 갖는 편이
> 좋습니다. 여러분이 러스트에 더 경험을 쌓게 되면 가장 효율적인
> 해답을 가지고 시작하기 더 쉽겠으나, 지금으로선 `clone`을 호출하는
> 것도 완전 허용될만 합니다.

`main`을 업데이트하여 `parse_config`이 반환한 `Config` 인스턴스를
...
이용하는 어떠한 코드라도 `config` 인스턴스에서 목적에 맞게 명명된 필드 값을
찾을 수 있습니다.
...
전달되어야 함을 알게 해 주었습니다. 그런 다음 `query`와 `file_path`의
...
위한 에러 메시지입니다. 우리의 최종 사용자들에게는 대신 무엇을 해야 하는지를
...
이번 출력이 더 좋습니다: 이제는 적절한 에러 메시지가 되었습니다. 하지만 사용자들에게
...
`Result`를 반환할 필요가 있게 된 함수 본문을 위해 필요한 변경점을
...
에러가 난 경우를 처리하여 사용자 친화적인 메시지를 출력하기 위해서는,
...
있습니다. `unwrap_or_else`의 사용은 커스터마이징 된 `panic!` 아닌 에러 처리를
...
우리가 정의하여 `unwrap_or_else`의 인수로 넘겨준 익명 함수입니다.
...
이제 환경 값 파싱의 리팩터링을 끝냈으니, 프로그램 로직으로 돌아와 봅시다.
[“바이너리 프로젝트를 위한 관심사의 분리”](#separation-of-concerns-for-binary-projects)<!-- ignore -->에서
기술한 바와 같이, 현재 `main` 함수에 있는 로직 중 환경 값
...
`expect`를 호출하여 프로그램이 패닉이 되도록 하는 대신, `run` 함수는
뭔가 잘못되면 `Result<T, E>`를 반환할 것입니다. 이것이 사용자 친화적인
방식으로 에러를 처리하는 `main` 안의 로직을 더 굳건히 만들게 해 줄
...
세 번째로 `run` 함수는 이제 성공한 경우 `Ok` 값을 반환합니다.
...
알아보는 검사를 안 하고 있고, 그래서 컴파일러가 어떤 에러 처리 코드를 의도했었던
...
`if let`과 `unwrap_or_eulse` 함수의 본문은 둘 모두 동일합니다:
에러를 출력하고 종료합니다.
...
하는 것으로 이 새로운 모듈성의 이점을 활용해 봅시다: 테스트를
작성해 보겠습니다!
